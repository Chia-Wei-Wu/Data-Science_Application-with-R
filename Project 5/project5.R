...
if (length(arg)!=6) {
  ...
  stop("Rscript hw6_111753141.R --train Data/train.csv --test Data/test.csv --predict studentID.csv", call.=FALSE)
}
...
#training
dtrain <- xgb.DMatrix(data = as.matrix(df_train[, -2]), label = df_train$SeriousDlqin2yrs)
dtest <- xgb.DMatrix(data = as.matrix(df_test[, -2]))

params <- list(objective = "binary:logistic", eval_metric = "logloss")
cv_result <- xgb.cv(params = params,data = dtrain,nrounds = 100,nfold = 5,early_stopping_rounds = 10, verbose = 0)
...
for (row in 1:length(probabilities)) {
  if (probabilities[row] < 0) {
    probabilities[row] <- 0
  }
  if (probabilities[row] > 1) {
    probabilities[row] <- 1
  }
}

submit <- data.frame(id = df_test[,1], probability = probabilities)
